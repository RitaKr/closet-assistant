...
import { EditButton, DeleteButton, ApplyButton, CancelButton } from "../components/Buttons";
...
export default function Collection({ collection }) {
    ...
    useEffect(() => {
   //console.log("filteredOutfits:", outfits)

	}, [outfits]);

    function sortOutfits() {
		setOutfits(_.orderBy(
			//filteredClothes,
			outfits,
			[(item) => (item[sort.by] === "" || item[sort.by] === undefined) ? "zzz" : item[sort.by].toLowerCase()],
			[sort.asc ? "asc" : "desc"]
		));
			...
			<main className="page-main">
				...
				<h1 className="page-title editable-title">
                    ...
                    (!editing ? (
                        ...
                        <div className="action-panel">
                        <ApplyButton handleApply={handleSubmit} disabled={!validCollectionName(name, collections)}/>
